...
for domain in domainList:
    ...
    for name in objectNames:
        ...
        for attr in mBeanAttrs :
            if not(attr.isWritable()) :
                print '  Attribute: ' + attr.getName() + ', type: ' + str(attr.getType()) + ', ' + name.toString() + ': is ReadOnly'
            if not(attr.isReadable()) :
                print '  Attribute: ' + attr.getName() + ', type: ' + str(attr.getType()) + ', ' + name.toString() + ': is Not Readable'
            if (attr.isWritable()) :
